...
static cl_command_queue queue_1;
static cl_command_queue queue_2;
...
int main(int argc, char **argv) {
    ...
    size_t binsize = 0;
    unsigned char * binary_file = loadBinaryFile("bin/r3_1_ul14m.aocx", &binsize); 
    if(!binary_file) {
    ...
    kernel_1 = clCreateKernel(program, "RANSAC_threshold_1", &status);
    if(status != CL_SUCCESS) {
      dump_error("Failed clCreateKernel.", status);
      freeResources();
      return 1;
    }
    kernel_2 = clCreateKernel(program, "RANSAC_threshold_2", &status);
    if(status != CL_SUCCESS) {
      dump_error("Failed clCreateKernel.", status);
      freeResources();
      return 1;
    ...
    for(int rep = 0; rep < p.n_warmup + p.n_reps; rep++) {
        ...
        clSetKernelArgSVMPointerAltera(kernel_1, 0, (void*)d_model_param_local);
		clSetKernelArgSVMPointerAltera(kernel_1, 1, (void*)d_flow_vector_array);
        clSetKernelArg(kernel_1, 2, sizeof(int), &n_flow_vectors); 
		clSetKernelArg(kernel_1, 3, sizeof(int), &p.max_iter);
		clSetKernelArg(kernel_1, 4, sizeof(int), &p.error_threshold);

        clSetKernelArgSVMPointerAltera(kernel_2, 0, (void*)d_model_param_local);
		clSetKernelArgSVMPointerAltera(kernel_2, 1, (void*)d_flow_vector_array);
        clSetKernelArg(kernel_2, 2, sizeof(int), &n_flow_vectors); 
		clSetKernelArg(kernel_2, 3, sizeof(int), &p.max_iter);
		clSetKernelArg(kernel_2, 4, sizeof(int), &p.error_threshold);
